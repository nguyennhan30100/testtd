...
//   của nó (tính đến HẾT một trận) đạt ≥ 50đ.
...
//   kích hoạt — nếu đội chỉ vừa đủ 50đ NHỜ điểm trận đó thì chưa
//   được tính là "đã kích hoạt" ở trận đó (phải đợi top1 ở trận
//   sau nữa mới được).
// - Một đội mới 30đ mà top1 thì KHÔNG thắng (chưa kích hoạt CPR).
...
// ============================================================
function computeChampionBoard(matchesOrdered) {
  const MAX_MATCHES = 5;
  const limited = matchesOrdered.slice(0, MAX_MATCHES);

  const teamTotals = {}; // key(cleanIds joined) -> { rep, by, kill, score, cleanIds }
  const cprActivated = new Set(); // key của các đội đã đạt ≥50đ (đã kích hoạt CPR)
  ...
  for (const match of limited) {
    ...
    // TỪ TRƯỚC trận này (tổng điểm tính đến HẾT TRẬN TRƯỚC đã ≥50đ) → vô địch ngay.
    // Lưu ý: điểm của chính trận top1 này KHÔNG được tính vào mốc kích hoạt —
    // nếu đội chỉ vừa đạt đủ 50đ NHỜ điểm của trận đang xét thì chưa được tính.
    ...
    // Bước 3: cập nhật tập "đã kích hoạt CPR" cho TẤT CẢ đội đạt ≥50đ TÍNH ĐẾN HẾT TRẬN NÀY
    // (sẽ có hiệu lực để xét top1 ở TRẬN SAU, không áp dụng cho trận này nữa)
    for (const key of Object.keys(teamTotals)) {
      if (teamTotals[key].score >= 50) cprActivated.add(key);
    }
  }
...
client.on("messageCreate", async (message) => {
  ...
  if (pending?.step === "template" && pending.channelId === message.channel.id) {
    ...
    const { accountId, soKhung, guildId: gid, mode, ngay, excludeMatchIndex } = pending;
    ...
    try {
      ...
      // ══════════════════════════════════════════════════════
      if (mode === "bxhcpr") {
        const { sortedTeams, hasChampion } = computeChampionBoard(matchDetails);
        ...
        const statusLine = hasChampion
          ? `👑 Đã có đội VÔ ĐỊCH (đạt ≥50đ & top1) sau ${usedMatches} trận — các trận sau không xét.`
          : `📊 Chưa có đội đủ điều kiện vô địch sau ${usedMatches} trận — xếp hạng theo tổng điểm.`;
        return loadMsg.edit({
  ...
  // .td [ID] [cpr] [xoaN]  — 1 lệnh duy nhất thay cho .td / .bxh / .bxhcpr cũ
  // - .td 60967899          → chế độ thường (embed + lưu data)
  // - .td 60967899 cpr      → chế độ "vô địch" (đội ≥50đ & top1 được đưa lên #1), không lưu data
  // - Thêm "xoaN" ở bất kỳ đâu sau ID để bỏ trận thứ N. VD: .td 60967899 cpr xoa3
  if (cmd === "td") {
    const accountId = args[1];
    if (!accountId || !/^\d+$/.test(accountId)) {
      return message.reply("❌ VD: `.td 60967899` hoặc `.td 60967899 cpr` (thêm `xoa3` để bỏ trận số 3)");
    }

    let isCpr = false;
    let excludeMatchIndex = null;
    for (const a of args.slice(2)) {
      if (/^cpr$/i.test(a)) { isCpr = true; continue; }
      const xoaMatch = XOA_REGEX.exec(a);
      if (xoaMatch) excludeMatchIndex = parseInt(xoaMatch[1]);
    }

    const mode = isCpr ? "bxhcpr" : "td";
    pendingStep[uid] = { step: "khung", accountId, channelId: message.channel.id, guildId, mode, excludeMatchIndex };
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: `ID: ${accountId}${isCpr ? " • chế độ CPR" : ""}${excludeMatchIndex ? ` • sẽ bỏ trận #${excludeMatchIndex}` : ""} • ${message.author.username}` });
    return message.reply({ embeds: [embed] });
  }
  ...
  if (cmd === "help") {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: "🗑️ Xoá trận lỗi", value: "Thêm `xoaN` sau ID (bất kỳ vị trí nào) để bỏ trận thứ N (theo thứ tự thời gian) ra khỏi danh sách tính điểm.\nVD: `.td 4252953187 cpr xoa3` → chế độ CPR, bỏ trận số 3, chỉ tính các trận còn lại." },
        { name: "⚙️ Luồng",      value: "1️⃣ `.td [ID]` (thêm `cpr` và/hoặc `xoaN` nếu cần) → 2️⃣ Chọn khung giờ (1-8) → 3️⃣ Nhập ngày → 4️⃣ Chọn mẫu ảnh (1, 2, 3 hoặc 4) → 5️⃣ Bot tự lấy điểm từ Garena!" },
      )
